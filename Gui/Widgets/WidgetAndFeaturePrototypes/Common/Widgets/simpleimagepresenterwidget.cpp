...
void SimpleImagePresenterWidget::setImageByPath(QString imagePath, ImageTypesEnum imageType) {
...
void SimpleImagePresenterWidget::setImageByByteArray(const QByteArray &bytes, ImageTypesEnum imageType)
{
    this->_currImageType = imageType;
//    this->bytes = bytes;
...
{
    this->_currImageType = PixelBased;
...
void SimpleImagePresenterWidget::_refreshPresentablePixmapByRenderImage()
{
    ...
    */
    ...
    // SVG
#if 1
    if(_currImageType == SvgBased && _svgRenderer.isValid())
    {
    ...
    // Pixel Based Image
    if(!_renderImage.isNull() && _currImageType == PixelBased)
    {
#if 0
