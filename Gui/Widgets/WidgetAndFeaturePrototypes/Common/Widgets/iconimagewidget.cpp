...
IconImageWidget::IconImageWidget(ApplicationStatesEnum initialState, QWidget *parent) :
...
//    QWidget::paintEvent(e); // draws the background etc.
    ...
    if(!this->_pixmapToPresent.isNull())
    {
        ...
        if(_relatedAppState == GrabbedButNotInstalled) {
...
void IconImageWidget::setRelatedAppState(ApplicationStatesEnum newState)
{
...
ApplicationStatesEnum IconImageWidget::getRelatedAppState() const
{
