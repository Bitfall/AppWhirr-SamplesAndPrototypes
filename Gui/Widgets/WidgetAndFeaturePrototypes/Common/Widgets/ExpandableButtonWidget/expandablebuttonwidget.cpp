#include "expandablebuttonwidget.h"
...
//    mainLayout->setSizeConstraint(QLayout::SetMinAndMaxSize); // makes it flickering
    ...
    this->expandCollapseImageWidget->setImageByPath(QString(":/test/intelligent_button-expand"));
...
IntelligentSeparatedMultifunctionalButtonWidget *IntelligentSeparatedMultifunctionalButtonWidget::createWithItems(IntelligentSeparatedMultifunctionalButtonWidgetOptionItemModel *defaultOptionItemModel, QList<IntelligentSeparatedMultifunctionalButtonWidgetOptionItemModel *> otherOptionItemModels, QWidget *parent)
{
    ...
    return new IntelligentSeparatedMultifunctionalButtonWidget(defaultOptionItemModel, otherOptionItemModels, collapsedSize, expandedSize, false, parent);
}

void IntelligentSeparatedMultifunctionalButtonWidget::paintEvent(QPaintEvent *e)
{
    qDebug(" - render (IntelligentSeparatedMultifunctionalButtonWidget)");
...
void IntelligentSeparatedMultifunctionalButtonWidget::expandCollapseWidgetClicked()
{
    ...
    if(this->getIsExpanded()) {
        this->switchToCollapsedState();

        int otherOptionCnt = this->otherOptionItemWidgets.size();
        for(int i = 0; i < otherOptionCnt; i++) {
            this->otherOptionItemWidgets[i]->switchToCollapsedState();
        }
    }
    else {
        this->switchToExpandedState();

        int otherOptionCnt = this->otherOptionItemWidgets.size();
        for(int i = 0; i < otherOptionCnt; i++) {
//            this->otherOptionItemWidgets[i]->show();
            this->otherOptionItemWidgets[i]->switchToExpandedState();
        }
    }
}
