...
//
// the get..DirectoryPath() methods have to create the directory if it does not exists. If it cannot create the directory it has to return "failed" (usually empty string)
class PathManager
{
private:
    PathManager();
...
public:    
    ...
    // [!] It's a recursive function
    //  [!] It won't affect symlink / shortcut files. Simply skips them.
    static bool copyWholeDirectory(QString sourceDirPath, QString destDirPath, bool isCopyOnlyContentOfSource);
    ...
    //
    // normalizes the path (replaces \s with /s, and cleans the path - which is QDir::cleanPath - resolves system-links, ., .., etc.
...
private:
    static bool copyWholeDirectory_recursive(QString sourceDirPath, QString destDirPath);
