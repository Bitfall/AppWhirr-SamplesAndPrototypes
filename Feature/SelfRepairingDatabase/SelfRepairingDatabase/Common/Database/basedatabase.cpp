...
DatabaseTableSchemeDescription BaseDatabase::_getCurrentDatabaseSchemeDescriptionForTable(QString tableName)
{
    DatabaseTableSchemeDescription databaseSchemeDescr;

    {
        ...
        if(!query.exec()) {
            ...
            while (query.next())
            {
                ...
                databaseSchemeDescr.addItem(DatabaseTableSchemeDescription::DatabaseTableSchemeDescriptionItem(cid, name, type));
...

    {
        ...
        if(isSucceeded) {
            ...
            for(int i = 0; i < queriesToExecuteInTransaction.size() && isSucceeded; i++) {
                QSqlQuery queryForDyn(db);
                queryForDyn.prepare(queriesToExecuteInTransaction[i]);
                if(!queryForDyn.exec()) {
