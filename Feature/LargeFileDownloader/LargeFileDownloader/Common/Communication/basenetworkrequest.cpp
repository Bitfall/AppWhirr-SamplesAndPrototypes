...
#ifdef ENABLE_COMMUNICATION_LOGGING
    DLog("Network request deleted with url: " + _requestUrlString.toString());
#endif
...
void BaseNetworkRequest::startThisRequest() {
    ...
    if(_requestUrlString.toString().startsWith("/")) {
        ...
        _requestUrlString = QUrl(_defaultRequestProtocolPrefix + _requestUrlString.toString());
    }
    _networkRequest->setUrl(_requestUrlString);
#ifdef ENABLE_COMMUNICATION_LOGGING
    DLog("Final network request url: ") << _requestUrlString.toString();
#endif
