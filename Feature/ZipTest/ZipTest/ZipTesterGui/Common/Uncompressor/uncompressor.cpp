...
bool Uncompressor::uncompress(QString targetZipFilePath, QString targetOutputDirectoryPath)
{
    // Delete directories
    ...
    for(bool more = zip.goToFirstFile(); more && !isFailedToUncompress; more = zip.goToNextFile()) {
        if(!zip.getCurrentFileInfo(&zipFileInfo)) {
            WLog(QString("testRead(): getCurrentFileInfo(): %1").arg(zip.getZipError()));
            return false;
        }
        ...
        QString path = PathHelper::combineAndCleanPathes(targetOutputDirectoryPath, zipFileInfo.name);
        ...
        if ((zipFileInfo.externalAttr & flagDir) == flagDir) {
            ...
            // File
...
#if defined(ENABLE_COMPRESSION_LOGGING)
                ...
                if (setupPermissions) {
                    ...
                    // Owner permissions
                    if ((zipFileInfo.externalAttr & S_IRUSR) == S_IRUSR)
                        filePermission |= QFile::ReadOwner | QFile::ReadUser;
                    if ((zipFileInfo.externalAttr & S_IWUSR) == S_IWUSR)
                        filePermission |= QFile::WriteOwner | QFile::WriteUser;
                    if ((zipFileInfo.externalAttr & S_IXUSR) == S_IXUSR)
                        filePermission |= QFile::ExeOwner | QFile::ExeUser;

                    // Group permissions
                    if ((zipFileInfo.externalAttr & S_IRGRP) == S_IRGRP)
                        filePermission |= QFile::ReadGroup;
                    if ((zipFileInfo.externalAttr & S_IWGRP) == S_IWGRP)
                        filePermission |= QFile::WriteGroup;
                    if ((zipFileInfo.externalAttr & S_IXGRP) == S_IXGRP)
                        filePermission |= QFile::ExeGroup;

                    // Other permissions
                    if ((zipFileInfo.externalAttr & S_IROTH) == S_IROTH)
                        filePermission |= QFile::ReadOther;
                    if ((zipFileInfo.externalAttr & S_IWOTH) == S_IWOTH)
                        filePermission |= QFile::WriteOther;
                    if ((zipFileInfo.externalAttr & S_IXOTH) == S_IXOTH)
